...
The framework harness requires from you a solid understanding of three things:

- enterprise software development lifecycle (SDLC) roles and processes
- the capability to lead, steer, course correct and develop engineering teams that live these processes to become meaningfully productive for you
- And the actual understanding of how software and building software works across business requirements, product management, project management, business analyst requirement capture, architecture, development work, code review, quality assurance, CI/CD devops, cybersecurity, scalability, availability, the lot.
...
It is for operators and teams whose work has to **meet mature, enterprise-grade SDLC principles and requirements**: review gates that are not bypassable, change records you can audit a year later, regression safety on a long-lived codebase, division of concerns across roles that actually do different things, and the ability to pause and resume cleanly.
...
- **context7.** Installed by default during onboarding via its preferred CLI + Skills mode (`ctx7 setup --cli --<agent>`) -- a user-global skill, no per-project config, and **no API key required** for documentation lookups (a free [context7.com](https://context7.com/) key is optional and only raises rate limits). A Node.js 18+ runtime is the only prerequisite. An MCP-server fallback exists for environments that can't run the CLI; that mode does need the API key. context7 is an AEH-standard tool; you can opt out per project with a deliberate reason, but the default path installs it.
...
2. **The harness tells you, loudly.** Open a harness session as `target-orchestrator` for a project. Session-init compares that project's `harness-sync-sha` against the new harness HEAD and, if it is behind, surfaces a prominent `UPGRADE REQUIRED` block as the first thing after the banner -- naming how many commits behind, stating not to start code work until upgraded, and pointing at one response: say `upgrade`. (You can defer explicitly, but the default posture is upgrade-first.)
3. **Run the one runbook.** Say `upgrade`. The harness drives a single turnkey, self-verifying procedure (`templates/playbooks/upgrade.md`) end-to-end via prompts dispatched **into that project's session**: refresh the base-persona / role / check-script snapshots from master, uplift the project's `CLAUDE.md` to the current template, run the behavioural-retrofit assessment (the former `review changes` Propagation-Impact pass, now one operator-gated step), drive the deterministic check clean, then bump `harness-sync-sha`. Every step has a hard verification gate; you approve the judgment step.
4. **Done means proven.** The runbook's terminal state is an explicit `UPGRADE COMPLETE -- synced to <HEAD>, aeh-practice-check clean, ready for code work`, after which the `UPGRADE REQUIRED` gate stops firing. The deterministic check (`docs/AE/bin/aeh-practice-check.sh .`) passing is your confirmation, not a vibe.
