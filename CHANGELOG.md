...
### Changed
- **README full restructure -- governance harness for mature SDLC needs** (`README.md` 342 -> 153 lines net). Repositions AEH from "Agentic Engineering Harness" to "Agentic Engineering Governance Harness" (G as clarifier, not a rename). Restructured into eight question-answering sections: (1) "What is this?" lead with positive disambiguation from fast-prototype tooling like Lovable, framed as "different problems, different categories; not in competition", and orientation toward "mature, enterprise-grade SDLC needs, principles, and requirements"; (2) "Why this exists" -- the problem AI agents create that this addresses; (3) "Inner mechanics" -- seven SDLC discipline bullets including spec-first with the "rigour of the proposal IS the quality of every downstream handoff" principle, OpenSpec as the spec substrate, context7 as current-docs lookup, TDD with spec-references-in-tests, restartable persistent state, evolve-the-system fixes; (4) "The roles" -- engineering personas table with key-discipline column + coordinating roles table (Orchestrator team-manager, Strategist external in any LLM chat, Harness Reviewer for self-review) + paragraph on the loop and PASS/WARN/FAIL/BLOCK verdicts that are auditable + two-session shape; (5) "Operation modes" -- conversational / operator-paced / autonomous chained spectrum, including e2e browser testing in autonomous mode; (6) "Onboarding modes" -- greenfield vs brownfield as starting points + the on-ramps table; (7) "Required infrastructure" -- sandbox + control-in-depth paragraph naming what AEH does NOT provide (sandboxing, CI/CD deploy gate, secrets management) so operators bring those independently; (8) "Quick start" -- 3-line clone + `onboard`. Plus brief "What AEH is not" factual category-clarification block, pointers to deeper docs, and footer. Voice: PostgreSQL/ripgrep/Vault README -- concrete, observational, not promotional. ASCII-only throughout per the harness rule.
