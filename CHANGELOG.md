...
### Changed
- **README rewrite -- governance reframing + flexibility-first framing** (`README.md` 342 -> 141 lines net). Repositions AEH from "Agentic Engineering Harness" to "Agentic Engineering Governance Harness" (G as clarifier of what AEH already is, not a rename). Lead-in describes AEH as "a working starting point ... designed to be molded toward the specific practices of your team, department, or company" -- explicitly invites adaptation rather than asserting unconditional rules. Two top-level sections do the work: "What you get out of the box" (six default disciplines as descriptive capability, not commandments) and "How you bend it" (six dimensions of amendability: persona overlays, reviewer cadence, dispatch mode, tool selection, maturity entry point, skip-what-you-don't-need). Drops the "look how clever this is" tone, drops project-specific brag content (no named downstream projects, no specific iteration counts), drops absolute claims about isolation -- replaced with "Most teams keep these separate ... Some teams collapse them and that works too if the audit trail is not a priority." Folds deep detail (prompt format, chain wrapper internals, directory trees, exhaustive role tables) into pointers to `CLAUDE.md`/`templates/personas/`/`docs/`. Audience: someone landing on the GitLab repo who needs to understand "what is this and should I care?" within 60 seconds. Voice: PostgreSQL/ripgrep/Vault README, not marketing landing page.
