...
### Changed
- **README rewrite — governance reframing** (`README.md` 342 → 138 lines, -279/+75) -- repositions AEH from "Agentic Engineering Harness" to "Agentic Engineering **Governance** Harness" (G as clarifier of what AEH already is, not a rename). Lead with role and responsibility for newcomers ("60-second pitch"), drop the "look how clever this is" tone, fold deep details (prompt format, chain wrapper internals, full directory tree, exhaustive role tables) into pointers to `CLAUDE.md`/`templates/personas/`/`docs/`. New "What 'governance' means here" section names the seven specific disciplines the harness imposes (multi-role pipeline, no code without governing spec, mandatory reviewer cadence, boundary-iteration until clean, evolve-the-system principle, two dispatch modes operator-paced vs chain execution, restartable state, role discipline). Audience: someone landing on the repo who needs to understand "what is this and should I care?" within 60 seconds. Voice: PostgreSQL/ripgrep/Vault README, not marketing.
