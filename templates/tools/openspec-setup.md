...
2. **Adds a provenance marker on the new content.** Each applied section gets a short marker (project-local phrasing; example: "reflects design intent, not yet re-verified against the live system"). The marker signals to the next reader that the content's verification status has not been re-established.

Cross-reference: the archaeologist persona is the canonical source for the verification convention; the close-out playbook preserves what the archaeologist established but does not re-establish it.
