...
- **CLAUDE.md router discipline (judgment).** `CLAUDE.md` is read in full every session BEFORE a role/task is chosen, so it must be a thin router, not a manual. For each section/bullet apply the test: *does every session need this BEFORE it knows its role/task?* If yes it belongs inline (isolation/fence, ASCII-only, no-AI-attribution, the capture principle, session-init/role-selection). If no, it is role/task-specific and should be a one-line rule + resolvable pointer to its owning home (a persona, playbook, openspec spec, `docs/AE/` reference). Inlined role/task-specific content is a finding (the size-budget WARN is the crude tripwire; this is the real signal). A soft size budget WARN also exists for targets via `bin/aeh-practice-check.sh`'s `claude-md-size`; the harness CLAUDE.md is checked here by judgment.
- **Pointer resolution.** Every `CLAUDE.md` (and persona) `Detail:`/`Full reference:`/`Full mechanism:` pointer resolves to an existing file/section -- a dangling pointer is the dual of inlined content and is a finding. (This is the content-side companion of the Dimension-4 "no harness-only path references in target-facing files" check, which catches pointers that will not resolve from a TARGET tree.)
