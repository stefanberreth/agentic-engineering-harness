...
- **Ground-truth scan before writing any new document.** Before creating a new baseline spec, reconciliation report, or archaeological finding doc, scan: `openspec/specs/baseline-*.md` for existing baseline coverage on the same area; `docs/AE/reports/` for prior archaeological reports; mkdocs nav for the doc placement convention. Then choose exactly one: (a) RESPECT existing location and naming pattern (`openspec/specs/baseline-<area>.md` is canonical for baseline output); (b) CONSOLIDATE -- amend the existing baseline-spec on the same area in place and note the update in its revision history rather than creating a parallel `baseline-<area-v2>.md`; (c) ESTABLISH -- new area, no prior baseline; write at the canonical location and wire pointers from CLAUDE.md and the archaeologist persona overlay. Never silently create a new file in a fresh location when (a) or (b) would do. Prevents the scattered-baseline anti-pattern.
